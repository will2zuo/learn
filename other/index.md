...
#### 限流：
- 计数器限流【固定窗口】：在固定时间内限制固定的请求数量，但是存在在重置节点突发【节点前后请求最大请求数量】请求超过限制，窗口切换时可能出现流量突刺
  - 计数器限流【滑动窗口】：将时间窗口细分为多个小周期（如10个100ms窗口），统计连续N个周期的总请求量，避免突刺问题
- 漏桶限流：进入桶的速率不限，桶的大小固定，消费的速度固定
- 令牌桶限流：桶的大小固定，令牌生成的速度固定，消耗令牌的速度不固定，每次请求都会从桶里取令牌，如果没有令牌就丢弃这次请求，能动态调整令牌发放的速度，针对突发流量

#### mysql 大数据量处理
- 架构的优化
  - 分表分库
    - 水平分表：按规则将大表拆分为多个小表，降低单表的数据量
    - 垂直分表：将字段按业务拆分到不同表，减少表的宽度
    - 分库：按业务模块分不同的数据库，减少单库的压力
  - 分区：按范围【range】/hash/list 等方式分区，提高查询的效率
  - 读写分离：主库写，从库读
- 查询和索引优化
  - 合理设计索引
  - SQL 查询语句优化
    - 避免 select * 
    - 用JOIN替代子查询，用EXISTS替代IN
- 冷热数据归档
- 增加缓存 redis/Memcached 等缓存热数据，减少对数据库的压力
...
#### 如何处理高并发
- 系统架构的优化
  - 分布式架构：微服务模块，降低单点故障的风险
  - 负载均衡：nginx 等方式分散请求，避免单点压力过大
  - 无状态设计：会话请求存储在 redis 中，方便横向扩展
- 缓存与异步处理
  - 多级缓存：cdn/redis 等减少数据库压力
  - 异步处理：耗时操作通过 mq 的方式异步处理，提高吞吐量
- 数据库优化
  - 分表分库：按业务或者模块拆分，降低单表压力
  - 读写分离
  - SQL 和索引的优化：避免全表扫描，使用覆盖索引
- 流量控制与容灾
  - 限流熔断：令牌桶或者漏桶算法限制请求速率
  - 多机房部署
- 监控和弹性扩容
  - 实时监控：天机阁/伽利略跟踪 CPU/内存/慢查询指标，设置告警阈值
  - 自动扩容：k8s 根据流量调整服务实例
...
- token 机制
  - 客户端请求服务带 token，服务端验证 token 存在才返回
    - token 生成和校验保证原子性【lua 脚本】
    - 先删除 token 在处理业务，避免并发问题
- 唯一索引或者约束
  - 数据库插入，避免脏数据
- 乐观锁【适用与更新操作】
- 悲观锁【适用强一致性场景】
  - 通过 SELECT ... FOR UPDATE 锁定数据行，确保串行执行
- 分布式锁【适用与分布式系统】
  - redis 的 setnx 或者 Zookeeper 实现
  - 注意点：需要设置过期时间，避免死锁
- 消息队列去重
  - 为消息分配唯一 id，消费前检查redis 或者数据库是否已经处理
- 幂等性设计的原则
  - 唯一标识：每个请求携带唯一 id
  - 记录处理状态：通过数据库、Redis 记录已处理的请求
  - 前置校验：处理前检查是否已执行过相同请求
...
### lua 脚本中一个命令失败，其他命令会回滚吗
不会
- 可以捕获异常，手动回滚
- 可以通过 watch 监控，若被修改则放弃操作
...
### 如何分析一条 sql 的性能问题
- explain 分析执行计划，查看是否用到索引，是否是全表扫描等
- 通过 SHOW PROFILE 定位耗时阶段，查看是否是 IO 瓶颈或者是临时表开销问题
- 开启慢日志查询分析
- 检查索引命中的有效性
- 分析锁和事务

### 高并发秒杀系统怎么设计，如何解决超卖的问题
- 前端：资源 cdn 分发，并设置验证码和防重复点击
- 接入层：nginx 负载均衡和单 ip 的限流（令牌桶）
- 服务层：
  - 秒杀服务，核心逻辑处理
  - 订单服务：异步处理
- 数据层：
  - redis 缓存数据，mysql 分表分库存储订单
- 超卖
  - redis 预热库存，并通过 lua 脚本原子性的减少库存，mysql乐观锁保证最终一致性
  - 异步订单生成
...
### websocket 瞬时大流量应该怎么应对
- 水平扩展：通过 Kubernetes 和负载均衡分散流量。
- 异步处理：用消息队列和 Goroutine 池缓冲请求。
- 资源优化：限制连接数、启用压缩、调整 OS 参数。
- 容灾降级：限流熔断、服务降级、多可用区部署。
- 监控压测：实时监控 + 定期压力测试。